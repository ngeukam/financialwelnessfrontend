...
import { AddCircle, Close, PanoramaRounded } from "@mui/icons-material";
...
const ManageCategories = () => {
    ...
    const getCategories = async () => {
        ...
        const result = await callApi({
            url: 'personalfinance/categories/', method: 'GET', params: {
    ...
    const generateColumns = (data) => {
        if (data.length > 0) {
            ...
            for (const key in data[0]) {
                if (key === 'children') {
                    columns.push({
                        field: key, headerName: key.charAt(0).toUpperCase() + key.slice(1).replaceAll("_", " "), width: 150, sortable: false, renderCell: (params) => {
